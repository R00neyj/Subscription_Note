...
const SortableHeader = ({ label, sortKey, width, sortConfig, onSort, isLast = false, children }) => {
  const getSortIcon = (key) => {
    ...
    return sortConfig.direction === 'asc' 
      ? <span className="text-white text-[12px] ml-1">▲</span> 
      : <span className="text-white text-[12px] ml-1">▼</span>
  }
  ...
  return (
    <th 
      className={cn(
        "px-2 text-center font-bold text-[14px] md:text-[16px] text-white leading-[1.4] cursor-pointer hover:bg-white/10 transition-colors select-none",
        width,
    ...
    <div className="w-full relative">
      ...
      <AnimatePresence>
        {showGuide && (
          <motion.div 
            ...
            className="absolute top-[-10px] left-4 md:left-20 z-[60] -translate-y-full"
          >
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full min-w-[700px] md:min-w-[800px] table-fixed border-collapse">
            <thead className="bg-dark dark:bg-slate-950">
              ...
              {/* Sortable Columns */}
              <SortableHeader label="서비스명" sortKey="service_name" width="w-[180px] md:w-[28%]" sortConfig={sortConfig} onSort={onSort}>
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <SortableHeader label="결제일" sortKey="billing_date" width="w-[110px] md:w-[15%]" sortConfig={sortConfig} onSort={onSort} />
              <SortableHeader label="결제 수단" sortKey="payment_method" width="w-[110px] md:w-[15%]" sortConfig={sortConfig} onSort={onSort} />
              <SortableHeader label="금액" sortKey="price" width="w-[110px] md:w-[15%]" sortConfig={sortConfig} onSort={onSort} />
              
              {/* head__item: 상태 */}
              <th className="w-[50px] md:w-[6%] text-center font-extrabold text-[14px] md:text-[16px] text-white leading-[1.4] whitespace-nowrap overflow-hidden text-ellipsis">
                상태
              </th>
            </tr>
          </thead>
          ...
          {data.map((item, index) => (
            ...
            >
              {/* 순번 */}
              <td className="w-[45px] md:w-[6%] border-r border-black/5 dark:border-slate-700/50 text-center font-bold text-dark/40 dark:text-slate-500 text-[13px] md:text-[17px] leading-[1.4]">
                {index + 1}
              </td>
              
              {/* 서비스명 */}
              <td className="w-[180px] md:w-[28%] border-r border-black/5 dark:border-slate-700/50 px-4 md:px-6">
                <div className="flex items-center gap-2 md:gap-3 justify-start">
                  ...
                  <div className="flex flex-col min-w-0">
                    <p className="font-extrabold text-dark dark:text-white text-[14px] md:text-[17px] leading-[1.4] truncate group-hover:text-primary transition-colors">
                      {item.service_name}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {item.is_free_trial && (
                        <div title="무료 체험 중" className="flex items-center justify-center p-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-md transition-colors group-hover:bg-blue-100">
                          <Timer size={13} strokeWidth={3} />
                        </div>
                      )}
                      {item.is_essential && (
                        <div title="필수 구독 항목" className="flex items-center justify-center p-1 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-md transition-colors group-hover:bg-purple-100">
                          <ShieldCheck size={13} strokeWidth={3} />
                        </div>
                      )}
                      {item.billing_cycle === 'yearly' && (
                        <div title="연간 결제 상품" className="flex items-center justify-center p-1 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-md transition-colors group-hover:bg-amber-100">
                          <CalendarDays size={13} strokeWidth={3} />
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <td className="w-[100px] md:w-[15%] border-r border-black/5 dark:border-slate-700/50 text-center px-1 md:px-2">
                 <div className="flex flex-wrap items-center justify-center gap-1 md:gap-1.5">
                    {item.categories?.map((cat) => (
                      <div key={cat} className="inline-flex items-center justify-center px-2 py-[4px] md:px-3 md:py-[6px] rounded-full bg-slate-100 dark:bg-slate-700 text-dark dark:text-slate-200 text-[11px] md:text-[14px] font-extrabold leading-none transition-all group-hover:bg-primary group-hover:text-white">
                        {cat}
                      </div>
                    ))}
              ...
              {/* 결제일 */}
              <td className="w-[110px] md:w-[15%] border-r border-black/5 dark:border-slate-700/50 text-center font-extrabold text-dark dark:text-white text-[13px] md:text-[17px] leading-[1.4]">
                {item.billing_date}
              </td>
              ...
              <td className="w-[110px] md:w-[15%] border-r border-black/5 dark:border-slate-700/50 text-center font-extrabold text-dark dark:text-white text-[13px] md:text-[17px] leading-[1.4]">
                <div className="flex flex-col">
                  ...
                  {item.billing_cycle === 'yearly' && (
                    <span className="text-[10px] md:text-[12px] text-dark/40 dark:text-slate-500 font-bold">
              ...
              {/* 상태 */}
              <td className="w-[50px] md:w-[6%] text-center">
                <div 
                  className={cn(
                    "w-[10px] h-[10px] md:w-[12px] md:h-[12px] rounded-full mx-auto shadow-sm transition-all duration-500",
      ...
      <div className="px-4 py-3 bg-slate-50/50 dark:bg-slate-900/30 border-t border-black/5 dark:border-slate-700/50 flex flex-wrap items-center gap-x-6 gap-y-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] md:text-[12px] font-bold text-slate-500 dark:text-slate-400">무료 체험 중</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="p-1 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-md">
            <ShieldCheck size={12} strokeWidth={3} />
          </div>
          <span className="text-[11px] md:text-[12px] font-bold text-slate-500 dark:text-slate-400">놓치면 안 될 필수 구독</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] md:text-[12px] font-bold text-slate-500 dark:text-slate-400">한 번에 결제하는 연간 상품</span>
        </div>
      </div>
    </div>
  </div>
